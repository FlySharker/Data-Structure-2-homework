...
protected:
	...
	void Delete3(BinTreeNode<ElemType> *&p); // 删除p指向的结点
...
public:
	...
	bool Delete2(const ElemType &key);						// 删除关键字为key的数据元素
	bool Delete3(const ElemType &key);						// 删除关键字为key的数据元素
...
// 操作结果: 删除p指向的结点
{
	...
	{ // p左右子非空
		...
		// 删除tmpPtr指向的结点
		if (tmpF->leftChild == tmpPtr) // 删除tmpF的右孩子
			Delete(tmpF->leftChild);
		else // 删除tmpF的左孩子
			Delete(tmpF->rightChild);
	}
...
template <class ElemType>
bool BinarySortTree<ElemType>::Delete2(const ElemType &key)
// 操作结果: 删除关键字为key的数据元素
{
	BinTreeNode<ElemType> *p, *f;
	p = Find(key, f);
...
// 操作结果: 删除p指向的结点
{
	BinTreeNode<ElemType> *tmpPtr, *tmpF, *u, *r;
	if (p->leftChild == NULL && p->rightChild == NULL)
	{ // p为叶结点
		delete p;
		p = NULL;
...
template <class ElemType>
bool BinarySortTree<ElemType>::Delete3(const ElemType &key)
// 操作结果: 删除关键字为key的数据元素
{
	BinTreeNode<ElemType> *p, *f;
	p = Find(key, f);
