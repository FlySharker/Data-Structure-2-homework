...
// 无向图的邻接矩阵类的实现部分
template <class ElemType, class WeightType>
AdjMatrixUndirGraph<ElemType, WeightType>::AdjMatrixUndirGraph(ElemType es[], int vertexNum, int vertexMaxNum)
...
template <class ElemType, class WeightType>
void AdjMatrixUndirGraph<ElemType, WeightType>::InsertArc(int v1, int v2, WeightType w)
// 操作结果：插入依附顶点v1和v2的边
{
	if (v1 < 0 || v1 >= vexNum)
		throw Error("v1不合法!"); // 抛出异常
